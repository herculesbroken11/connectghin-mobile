# Google Play store listing — final (verified against code)

**App name:** Connectghin  
**Package:** com.connectghin.app  

Do **not** use: Premier, best, #1, leading, Official GHIN, GHIN Verified, USGA verified, Discover/Pair Up/Find Your 4th, See who likes you, Profile Boost, message anyone.

## Short description

Find nearby golfers, Connect, Match, and browse open spots on The Feed.

## Full description

Connectghin helps golfers find playing partners near them.

WHAT YOU CAN DO

• Create a golfer profile with photos, location, and golf preferences  
• Browse nearby golfers on Connect and send a Connect like  
• When two golfers Connect with each other, they Match and can message in the app  
• Browse The Feed for golfers posting open spots for upcoming rounds  
• Submit handicap information for a Handicap Verified badge after review  
• Rate playing partners and report or block users when needed  

FREE FEATURES

• Profile, Connect, Matches, and messaging with your Matches  
• A limited preview of posts on The Feed  
• A daily limit on new Connect likes (shown in the app)  

PREMIUM MEMBERSHIP

Premium includes:

• Unlimited Connect likes  
• Full access to The Feed  
• Posting open spots on The Feed  
• Contacting golfers who posted open spots  
• A Premium badge on your profile  

LOCATION

Connectghin uses your location, with permission, to show nearby golfers and relevant Feed activity.

HANDICAP VERIFICATION

You may submit handicap information for review. If approved, a Handicap Verified badge can appear on your profile. This is not official third-party verification.

ACCOUNT & PRIVACY

Users can access Terms and Privacy information and request account deletion. Store subscriptions are managed separately through Google Play.

Connectghin is intended for adults 18 and older. Users confirm they are 18+ when creating an account, and profile age must be 18 or older to use core features.

## Feature graphic recommended text

Find Golf Partners Near You

## Screenshot captions (suggestions)

1. Home — Connectghin  
2. Connect — nearby golfers  
3. Golfer profile  
4. The Feed — open spots  
5. Matches  
6. Chat with a Match  
7. Premium benefits  

## Premium wording (only)

Unlimited Connect likes · Full Feed access · Post open spots · Contact Feed posters · Premium badge
